...
class ExtractionField(BaseModel):
    ...
    unit: Optional[str] = Field(
        default=None, description="Expected unit if applicable (e.g., USD, EUR)"
    )
...
class ExtractedDataPoint(BaseModel):
    ...
    notes: Optional[str] = Field(
        default=None, description="Additional notes about conversion or normalization"
    )
...
class AnalysisNodeResult(BaseModel):
    ...
    extracted_data: List[ExtractedDataPoint] = Field(
        default_factory=list, description="Successfully extracted data points"
    )
...
class AnalysisNode(BaseModel):
    ...
    extraction_fields: List[ExtractionField] = Field(
        default_factory=list, description="Specific fields to extract"
    )
    ...
    sub_tasks: List[AnalysisNode] = Field(
        default_factory=list, description="Child nodes for hierarchical analysis"
    )
    status: AnalysisNodeStatus = Field(
        default=AnalysisNodeStatus.PENDING, description="Current lifecycle status"
    )
    ...
    def get_must_have_fields(self) -> List[ExtractionField]:
        ...
        return [
            f for f in self.extraction_fields if f.priority == FieldPriority.MUST_HAVE
        ]
    ...
    def get_nice_to_have_fields(self) -> List[ExtractionField]:
        ...
        return [
            ...
            if f.priority == FieldPriority.NICE_TO_HAVE
        ]
...
class GlobalConstraints(BaseModel):
    ...
    language: str = Field(
        default="en", description="Preferred language for search queries"
    )
    date_format: str = Field(
        default="%Y-%m-%d", description="Standard date format to use"
    )
    ...
    allowed_regions: List[str] = Field(
        default_factory=list, description="Geographic regions to focus on"
    )
...
class ICPAnalysisInput(BaseModel):
    ...
    root_nodes: List[AnalysisNode] = Field(
        default_factory=list, description="Top-level analysis nodes"
    )
    created_at: datetime = Field(
        default_factory=datetime.now, description="Job creation timestamp"
    )
    input_file_path: Optional[str] = Field(
        default=None, description="Path to the source input file"
    )
    ...
    def get_pending_nodes(self) -> List[AnalysisNode]:
        ...
        return [
            n for n in self.get_all_nodes() if n.status == AnalysisNodeStatus.PENDING
        ]
...
class ICPAnalysisOutput(BaseModel):
    ...
    successful_nodes: int = Field(
        default=0, description="Number of successful extractions"
    )
...
class ConstitutionalMemory(BaseModel):
    ...
    normalize_currency: bool = Field(
        default=True, description="Whether to normalize currency to target constraint"
    )
...
class WorkingMemory(BaseModel):
    ...
    current_validation_result: Optional[Any] = Field(
        default=None, description="Result of the current data validation"
    )
    missing_fields: List[str] = Field(
    ...
    retry_reason: Optional[str] = Field(
        default=None, description="Reason for retry if in retry flow"
    )
...
class WorkflowMemory(BaseModel):
    ...
    is_icp_workflow: bool = Field(
        default=False, description="Whether this is an ICP analysis workflow"
    )
    nodes_processed: int = Field(
        default=0, description="Number of nodes that have been processed"
    )
    nodes_successful: int = Field(
        default=0, description="Number of successfully completed nodes"
    )
    nodes_failed: int = Field(default=0, description="Number of failed nodes")
...
class EpisodicMemory(BaseModel):
    ...
    search_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="History of search queries and results"
    )
    extraction_history: List[Dict[str, Any]] = Field(
        default_factory=list, description="History of data extractions"
    )
    error_log: List[Dict[str, Any]] = Field(
        default_factory=list, description="Log of errors encountered"
    )
...
class SemanticMemory(BaseModel):
    ...
    global_constraints: Optional[Dict[str, Any]] = Field(
        default=None, description="Global constraints for the ICP analysis"
    )
    analysis_input: Optional[Any] = Field(
        default=None, description="The full ICP analysis input specification"
    )
    extracted_knowledge: Dict[str, Any] = Field(
        default_factory=dict, description="Accumulated extracted data keyed by node ID"
    )
...
class ProceduralMemory(BaseModel):
    ...
    available_search_providers: List[str] = Field(
        default_factory=lambda: ["perplexity"], description="Available search providers"
    )
...
class ResourceMemory(BaseModel):
    ...
    api_calls_made: int = Field(
        default=0, description="Number of API calls made in this session"
    )
    api_call_limit: Optional[int] = Field(
        default=None, description="Maximum API calls allowed (None = unlimited)"
    )
    last_api_call_time: Optional[datetime] = Field(
        default=None, description="Timestamp of the last API call"
    )
