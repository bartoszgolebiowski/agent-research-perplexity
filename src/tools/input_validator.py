...
class ValidationRequest(BaseModel):
    ...
    json_content: Optional[str] = Field(
        default=None,
        description="JSON string content to validate"
    )
...
class ValidationResponse(BaseModel):
    ...
    analysis_input: Optional[ICPAnalysisInput] = Field(
        default=None,
        description="Parsed input if validation succeeded"
    )
...
class InputValidatorClient:
    ...
    def validate(self, request: ValidationRequest) -> ValidationResponse:
        ...
        try:
            if request.file_path:
                ...
                raise ValidationError("Either file_path or json_content must be provided")
    ...
    def _validate_semantics(self, input_spec: ICPAnalysisInput) -> None:
        ...
        if errors:
            raise ValidationError(
                f"Semantic validation failed with {len(errors)} error(s)",
                errors=errors
            )
